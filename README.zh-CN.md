...
- Stage 5 已拆分计划：先做 DPO 数据准备，再做 tiny DPO smoke test，再做行为对比，最后才考虑扩大 DPO。
- 学习型 notebook 已更新：`notebooks/04_full_pipeline_learning.ipynb`。

还没完成：

- Stage 5A：DPO preference 数据集。
...
现在进入 Stage 5 前的准备状态，但还没有开始 DPO 训练。

当前推荐仍然是 `outputs/sft_lora_qwen05b_custom_v3_from_v1_patch`。v4 没修好第 7 个 prompt，v5 修好了但破坏旧能力，v6 仍然不稳。下一步先做 `Stage 5A` tiny DPO preference 数据，再做 `Stage 5B` tiny DPO smoke test，并记录专用显存、共享显存、系统内存和 step 速度。tiny 可接受后，再考虑朴素或更完整的 DPO。
