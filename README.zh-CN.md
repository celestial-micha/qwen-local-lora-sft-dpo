...
现在已经停在 DPO 前。最适合做的是先检查和理解 Stage 2B.3：

当前推荐仍然是 `outputs/sft_lora_qwen05b_custom_v3_from_v1_patch`。v4 没修好第 7 个 prompt，v5 修好了但破坏旧能力，v6 仍然不稳。下一步我们应该一起复盘：是接受 v3 作为 DPO 前 SFT checkpoint，还是再做一轮更宽的 SFT replay。复盘前不要开始 DPO。
