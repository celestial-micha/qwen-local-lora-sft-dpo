...
- Stage 2B：自采集/自整理技术数据完成并根据训练反馈修订；Stage 2B.2 badcase patch 后当前是 126 train / 14 eval。
...
- Stage 2B.3：继续针对“为什么不能只看 loss”这个 remaining badcase 做更小、更稳的数据补丁。
- DPO 数据集和 tiny DPO smoke test。
- 多卡训练说明或实验。
...
现在最适合做的不是马上 DPO，而是先检查和理解 Stage 2B.2 的结果，然后针对最后一个 remaining badcase 做更小的补丁：

- 为什么不能只看 loss 判断 SFT 是否成功。

这次 Stage 2B.2 的重要经验是：小补丁从头训练可能会破坏原本已经学好的行为；从已有好 adapter 低学习率续训更稳。等固定 prompt 回归测试更稳定以后，再进入 tiny DPO smoke test 会更稳。
