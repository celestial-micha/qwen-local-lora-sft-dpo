...
当前 tiny DPO 系列证明 8GB 本地环境可以跑 DPO，小数据甚至 57 pair / 2 epoch 都没有 OOM。但行为质量没有过关：v2 仍没修好第 7 个 prompt，v3 推得更强后破坏了多个旧能力。因此当前不建议扩大 DPO；推荐 checkpoint 仍是 `outputs/sft_lora_qwen05b_custom_v3_from_v1_patch`。
