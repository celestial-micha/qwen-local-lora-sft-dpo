...
- Stage 5C：tiny DPO 固定 prompt 行为对比。
- 多卡训练说明或实验。
...
Stage 5A/5B 已完成，但还没有做 DPO 后行为对比。

当前 tiny DPO 输出是 `outputs/dpo_lora_qwen05b_tiny`。下一步是 Stage 5C：用同一批固定 prompt 对比 base、public-SFT、custom-SFT v3 和 DPO-tiny，确认第 7 个 prompt 是否改善，并检查旧能力是否回归。通过 Stage 5C 前，不扩大 DPO。
