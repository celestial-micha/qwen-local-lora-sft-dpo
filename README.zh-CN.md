...
Stage 5A 已完成，但还没有开始 DPO 训练。

当前推荐仍然是 `outputs/sft_lora_qwen05b_custom_v3_from_v1_patch`。v4 没修好第 7 个 prompt，v5 修好了但破坏旧能力，v6 仍然不稳。下一步需要先确认是否进入 `Stage 5B` tiny DPO smoke test，并记录专用显存、共享显存、系统内存和 step 速度。tiny 可接受后，再做 Stage 5C 固定 prompt 对比，最后才考虑扩大 DPO。
