...
- Stage 2B：自采集/自整理技术数据完成并根据训练反馈修订，当前是 119 train / 13 eval。
...
- Stage 2B.2：针对剩余两个 badcase 的小数据补丁。
...
现在最适合做的不是马上大规模 DPO，而是先检查和理解 Stage 3B/4B 结果，然后针对两个 remaining badcase 做小补丁：

- 为什么 public-SFT 没修正概念，反而说明 Stage 2B 有必要。
- 为什么不能只看 loss 判断 SFT 是否成功。

这两个补上以后，再进入 tiny DPO smoke test 会更稳。
