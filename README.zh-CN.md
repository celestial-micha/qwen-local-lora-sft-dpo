...
Stage 5A/5B/5C 已完成，但 Stage 5C 行为 gate 没过。

当前 tiny DPO 输出是 `outputs/dpo_lora_qwen05b_tiny`。它证明 8GB 本地环境可以跑 tiny DPO，但没有证明行为质量。下一步不建议扩大 DPO；应先修 Stage 5A preference 数据，尤其是第 4 个 public-SFT motivation 和第 7 个 loss-vs-behavior prompt，再重新跑 tiny DPO 和 Stage 5C。
