...
这个目录是专门为了学习和讲解本项目而创建的，不是训练产物目录，也不是必须运行的生产流程。它适合在上传 GitHub 后让读者按 notebook 顺序理解：Qwen 如何加载、LoRA 如何实现、SFT loss 如何计算、DPO preference 数据如何训练，以及为什么最终使用行为门禁选择 checkpoint。
